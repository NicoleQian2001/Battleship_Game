...
import cs349.a3battleship.view.IView
...
class Game (var dimension : Int, private var debug : Boolean) {
    ...
    var isExitGame = false // is set to true if Exit button is clicked
     ...
     */
    val onPlayerSetupBegin = ArrayList<(Player) -> Unit>()  // list of functions
    ...
    fun attackCell(cell : Cell)  {
        // perform attack
        // receivingPlayer is opponent player
        // activePlayer is current player
        var receivingPlayer = when (activePlayer) {
            Player.Human -> Player.AI
            Player.AI -> Player.Human
        }
        boards[receivingPlayer]!!.attackCell(cell) // when you click, the right board got attacked
    ...
    fun placeShip(player : Player, shipType : ShipType, orientation : Orientation, bowCell : Cell) : Ship? {
        println("bowCell: ${bowCell.x} , ${bowCell.y}")
